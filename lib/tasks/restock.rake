  ...
  task check_quantity_and_send_client_email: :environment do
    ...
    tenants.each do |tenant|
      Apartment::Tenant.switch(tenant) do
        ...
        restock_cases_group_by_client = Mycase.restocks.status_new.group_by { |m| m.client_id.to_s }
        ...
        if ms&.status && !ms&.product_xml.blank? && restock_cases_group_by_client.present?
          ...
          RestockJob.perform_later(tenant, restock_cases_group_by_client, ms.product_xml)
        else
