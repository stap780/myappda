class Restock::SendMessage < ApplicationService
    ...
    def initialize(tenant, restock_cases_group_by_client, xml_file)
      @tenant = tenant
        ...
        if events.exist?
          ...
          @restock_cases_group_by_client.each do |rcg| # this is clients iteration
            ...
            events.each do |event|
              ...
              if channel == 'email'
                if action.template.receiver == "client"
                  receiver = client.email
                else
                  receiver = action.template.receiver.blank? ? user.email : action.template.receiver
                end
